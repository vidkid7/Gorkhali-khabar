    ...
    <>
      ...
      <main className="min-h-screen" style={{ background: "var(--background)" }}>
        <div className="mx-auto max-w-6xl px-4 py-8">
          ...
          <PublicPageHeader title={ne("सेयर बजार (NEPSE)", "Share Market (NEPSE)")} eyebrow="NEPSE" description={ne("नेपाल स्टक एक्सचेन्ज — बजार डाटा", "Nepal Stock Exchange — Market Data")} countLabel={lastUpdate ? `${ne("अन्तिम अपडेट", "Last updated")}: ${lastUpdate.toLocaleTimeString()}` : undefined} breadcrumbs={[{ label: ne("गृहपृष्ठ", "Home"), href: "/" }, { label: ne("सेयर बजार", "Share Market") }]} actions={<div className="flex items-center gap-3">
              {/* Market status */}
          ...
          {data.indices.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
              {data.indices.map((idx) => (
                <div key={idx.name} className="utility-panel p-4"
