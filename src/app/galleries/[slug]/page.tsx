  ...
  if (!gallery) {
    return (
      <>
        <Header />
        <main className="mx-auto max-w-6xl px-4 py-6">
          <EditorialEmptyState title="ग्यालेरी भेटिएन" description="यो ग्यालेरी उपलब्ध छैन वा हटाइएको हुन सक्छ।" action={{ label: "फोटो ग्यालेरी", href: "/galleries" }} />
        </main>
        <Footer />
      </>
    );
    ...
    <>
      ...
      <main className="public-page-shell mx-auto max-w-6xl px-3 py-6 sm:px-4 sm:py-10">
        ...
        {gallery.images.length > 0 ? (
          <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 md:gap-4">
            {gallery.images.map((img, idx) => (
              <button
                ...
                className="group relative aspect-[4/3] cursor-pointer overflow-hidden rounded-[6px]"
          ...
          <div className="mt-8"><EditorialEmptyState title="कुनै फोटो छैन" description="यस ग्यालेरीमा अहिले फोटो उपलब्ध छैन।" /></div>
        )}
      </main>
      <Footer />
