    ...
    <>
      ...
      <div style={{ background: "var(--background)", minHeight: "100vh" }}>
        <div className="mx-auto max-w-7xl px-4 py-8">
          <PublicPageHeader title={language === "ne" ? "दैनिक राशिफल" : "Daily Horoscope"} eyebrow="Horoscope" description={language === "ne" ? "आफ्नो राशि छान्नुहोस् र आजको राशिफल पढ्नुहोस्।" : "Select your zodiac sign and read today's horoscope."} countLabel={today || undefined} breadcrumbs={[{ label: language === "ne" ? "गृहपृष्ठ" : "Home", href: "/" }, { label: language === "ne" ? "राशिफल" : "Horoscope" }]} />
          {/* Tab Bar */}
          <div className="mt-6 flex w-fit gap-2 border-b border-border">
            {TABS.map((t) => (
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 sm:gap-3 mb-8">
            {ZODIAC_SIGNS.map((sign) => (
              <button
                ...
                className="rounded-[6px] border-b-2 p-4 text-center transition-colors cursor-pointer"
                style={{
                  background: selected === sign.id ? "var(--surface-alt)" : "var(--surface)",
                  borderColor: selected === sign.id ? "var(--primary)" : "var(--border)",
                  color: selected === sign.id ? "var(--primary)" : "var(--foreground)",
                }}
              >
          ...
          {selectedSign && (
            <div className="utility-panel mb-8 overflow-hidden animate-fadeIn"
              style={{ border: `2px solid ${selectedSign.color}40`, background: "var(--surface)" }}>
              <div className="border-b border-border bg-ink p-6 text-white">
          ...
          <div className="utility-panel overflow-hidden">
            <div className="border-b border-border px-6 py-4" style={{ background: "var(--primary)", color: "#fff" }}>
