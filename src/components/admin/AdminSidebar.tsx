...
const navItems = [
  ...
  { path: "/quick-links",  labelNe: "द्रुत लिंक",     labelEn: "Quick Links",    icon: <Zap className="h-5 w-5" />, roles: ["ADMIN", "EDITOR"] },
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Link href={adminPath()} className="min-w-0 flex-1" style={{ color: "var(--foreground)" }}>
            {config.site_logo ? (
              <span className="inline-flex max-w-full rounded-[4px] bg-white px-2 py-1">
                <BrandWordmark compact className="w-[150px]" />
              </span>
            ) : (
              <span className="block truncate text-sm font-bold">
                {config.site_name?.en || "Gorkhali Khabar"}
              </span>
            )}
            <span className="mt-1 block truncate text-[10px]" style={{ color: "var(--muted)" }}>
              {lang === "ne" ? "प्रशासन प्यानल" : "Administration panel"}
            </span>
          </Link>
