...
export function Header() {
  ...
  useEffect(() => {
    if (!sidebarOpen) {
      if (sidebarWasOpenRef.current) menuButtonRef.current?.focus();
      sidebarWasOpenRef.current = false;
      return;
    }

    sidebarWasOpenRef.current = true;
    const focusFrame = requestAnimationFrame(() => drawerCloseButtonRef.current?.focus());
  ...
  return (
    <>
    <header className="site-header sticky top-0 z-50" style={{ background: "var(--header-bg)", borderBottom: "1px solid var(--border)", backdropFilter: "blur(14px) saturate(180%)", WebkitBackdropFilter: "blur(14px) saturate(180%)" }}>

      {/* ══════════ TIER 1: Utility row — hidden on scroll ══════════ */}
      <div
        className="site-utility-row hidden overflow-hidden border-b border-border transition-all duration-300 md:block"
        style={{ maxHeight: scrolled ? 0 : 48, opacity: scrolled ? 0 : 1 }}
      >
        <div style={{ height: 3, background: "var(--accent)" }} />
        <div className="mx-auto flex min-h-[42px] max-w-7xl items-center justify-between gap-4 px-3 sm:px-4">
          <div className="flex min-w-0 items-center gap-3 text-xs text-muted" style={{ fontFamily: "var(--font-latin)" }}>
            ...
            <div id="weather" className="hidden scroll-mt-32 lg:block">
              <WeatherWidget />
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className="rounded-[4px] border border-border px-2.5 py-1 text-xs font-bold text-foreground hover:bg-surface-alt"
            ...
            <button
              onClick={toggleTheme}
              className="rounded-[4px] p-2 text-muted-foreground hover:bg-surface-alt hover:text-foreground"
              aria-label={theme === "light" ? t("common.darkMode") : t("common.lightMode")}
            >
              {theme === "light" ? (
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M21 12.79A9 9 0 1 1 11.21 3a7 7 0 0 0 9.79 9.79z" /></svg>
              ) : (
      ...
      {/* ══════════ TIER 2: Masthead (removed) — logo now lives in the primary nav ══════════ */}


      {/* ══════════ TIER 3: Primary navigation ══════════ */}
      <div
        className={`site-primary-nav ${scrolled ? "is-sticky-compact" : ""}`}
        style={{
          background: "var(--nav-bg)",
          borderBottom: "1px solid var(--border)",
        }}
      >
        <div className="mx-auto max-w-7xl px-3 sm:px-4 flex items-center justify-between gap-3 min-h-[68px] py-1.5">

          {/* Brand wordmark — always visible in the primary nav */}
          <Link
            href="/"
            aria-label={siteName}
          ...
          <nav className="hidden lg:flex items-center min-w-0" aria-label="Main navigation">
            ...
            <div className="relative" ref={moreRef}>
              <button
                ...
                className="site-more-button px-3 py-3 font-semibold text-sm text-nav-text hover:text-accent transition-colors whitespace-nowrap flex items-center gap-1"
              >
                {t("nav.more")}
                ...
                <div className="absolute top-full left-0 mt-1 w-56 rounded-xl shadow-xl py-1.5 z-50 border animate-slideDown bg-surface border-border">
                  {MORE_NAV.map((item) => (
                    <Link
                      key={item.key}
                      href={item.href}
                      className={`block px-4 py-2.5 text-sm hover:bg-surface-alt hover:text-accent transition-colors ${isNavActive(item.href) ? "text-accent" : "text-foreground"}`}
                      aria-current={isNavActive(item.href) ? "page" : undefined}
          ...
          <div className="site-primary-nav__controls flex items-center gap-2 ml-auto shrink-0">
            ...
            <button
              onClick={() => setLanguage(language === "ne" ? "en" : "ne")}
              className="rounded-md border border-border px-2.5 py-1 text-xs font-bold text-foreground transition-colors hover:bg-surface-alt md:hidden"
              aria-label={t("common.language")}
            >
              {language === "ne" ? "EN" : "NE"}
            </button>
            ...
            <button
              onClick={toggleTheme}
              className="rounded-md p-2 text-muted-foreground transition-colors hover:bg-surface-alt hover:text-foreground md:hidden"
              aria-label={theme === "light" ? t("common.darkMode") : t("common.lightMode")}
            >
              {theme === "light" ? (
      ...
      {/* ══════════ TIER 4: Trending topics and search ══════════ */}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 h-14 border-b border-border shrink-0" style={{ background: "var(--accent)" }}>
              ...
              <button ref={drawerCloseButtonRef} onClick={() => setSidebarOpen(false)} className="p-2 rounded hover:bg-white/20 text-white" aria-label={t("common.close")}>
